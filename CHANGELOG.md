...
### Fixed — retrieval ranking and query latency

- **Full-text results are ordered by rank, not recency.** `query_blocks(text_search=...)` on every backend (SQLite FTS5, PostgreSQL, async PostgreSQL) sorted matches by `created_at DESC` regardless of match quality, so the top hit for `"Kotak XIRR"` was whichever row mentioning *either* word was written most recently (memblock OR-joins the query words). The storage contract now accepts `filters["sort_by"] = "relevance"`, which is also the default for text searches: SQLite orders by FTS5 `rank` (bm25, best first), PostgreSQL by `ts_rank(content_tsv, to_tsquery('english', …)) DESC` with `created_at DESC` as the tiebreak. Explicit `created_at` / `access_count` / `confidence` keep their existing `ORDER BY`. Non-text queries are unchanged.

- **Keyword rank now reaches the relevance score.** Without an embedding provider (or with `semantic=False`) the semantic term of `relevance_score` was 0 for every block, so FTS-only results were re-sorted purely by confidence/recency and the bm25 order from storage was discarded. When `text_search` is set and no hybrid scores exist, both query engines (`query.py` and `async_query.py`, kept behaviourally identical) now derive the hybrid score from the FTS order alone — `weighted_rrf_merge(fts_ids, [], None, k=3)` normalised so the first candidate scores 1.0 — so the best keyword match is the top result. The FTS-only curve deliberately uses RRF `k=3` instead of the standard `k=60`: after normalisation `k=60` separates rank 0 from rank 1 by 0.016 (0.011 in the final score, which weights the semantic term 0.70) while recency and strength together can move a block by 0.10, so a fresh one-token match still beat a 30-day-old exact match; with `k=3` (rank 1 → 0.80, rank 2 → 0.67, rank 9 → 0.31) the best keyword match wins on rank alone for equal confidence at any age, and only a much higher-confidence block can edge out an adjacent rank. The hybrid path keeps `k=60` because the vector side supplies score magnitude there. Reranker and `include_decayed` logic are untouched (for FTS-only searches the steep curve also means `related_to` graph proximity and confidence can no longer reorder the top keyword ranks — keyword rank is meant to dominate there); with hybrid search the FTS side of the RRF merge is now genuinely rank-ordered.

- **Candidate `LIMIT` is pushed down to SQL with an FTS-first join.** A hot query over a 10,000-block corpus matched ~2,800 rows and every one of them was hydrated into a `Block` before the engine kept 10; the SQL itself was a small fraction of the cost. Text-search queries now ask storage for `sort_by="relevance"` and `limit = max(limit * 5, 50)` (the pool `_hybrid_search` already assumed), and the `LIMIT` is applied after every WHERE condition (type, parent, confidence, tags, session/org/project/agent, metadata, temporal range, deleted) so a filtered query never under-fills. On SQLite the query *shape* had to change to make this safe: appending `LIMIT` to the old `FROM blocks b LEFT JOIN block_metadata m … INNER JOIN blocks_fts fts … WHERE fts.blocks_fts MATCH ?` statement makes the planner drive from `blocks` via `idx_blocks_deleted` and re-run the FTS `MATCH` once per row — measured at over 150 seconds for a single query at 10k rows. The new shape puts the match in a derived table on the left (`FROM (SELECT block_id, rank FROM blocks_fts WHERE blocks_fts MATCH ?) fts CROSS JOIN blocks b ON b.id = fts.block_id LEFT JOIN block_metadata m …`); SQLite honours `CROSS JOIN` as a loop-order hint, so the FTS scan is the single outer loop, verified with `EXPLAIN QUERY PLAN` for the unfiltered, `type`, `session_id`, `min_confidence`, `tags`, `metadata_filters` and `happened_after` cases (first plan row `SCAN blocks_fts VIRTUAL TABLE INDEX 32:M3` in every case, `idx_blocks_deleted` never appears, and `ORDER BY fts.rank` needs no temp b-tree so FTS5 honours the LIMIT inside the scan). PostgreSQL joins `blocks` and `block_metadata` in one WHERE, so `ORDER BY ts_rank(...) DESC LIMIT n` on the same statement is correct there (sync and async adapters, single round trip).

- **numpy fast path for the SQLite brute-force vector search.** The pgvector-less fallback unpacked every stored embedding with `struct` and ran a pure-Python cosine per row — ~0.4 s per query at 10,000 × 384 dims, before hydration. New `memblock.embeddings.brute_force_similarity(query_vec, embeddings)` returns every `(block_id, cosine)` pair sorted descending; when numpy is importable it reads the float32 blobs with `np.frombuffer(…, dtype="<f4")` (`pack_embedding` writes native float32, so big-endian hosts fall back), stacks them, L2-normalises with the same zero-norm guard as `cosine_similarity`, and does a single matmul (~0.3 ms). Without numpy, or with mixed-length blobs, it uses the existing pure-Python loop; the two paths agree within 1e-5 and produce the same ordering. Used by both query engines' fallback branch. numpy stays optional — no change to core dependencies — and `cosine_similarity` is unchanged.

- **Hybrid queries no longer hydrate every embedded block.** After the RRF merge both engines fetched every block the merge mentioned that was not already an FTS candidate. On the Python vector fallback `weighted_rrf_merge` adds a cosine bonus for *every* embedded block, so each hybrid query on a store without pgvector called `get_block` once per embedded block — ~10,000 fetches (≈330 ms) at 10k blocks, which hid most of the numpy win. The engines now hydrate at most the top `max(limit * 5, 50)` entries of the merged list: everything below trails at least that many higher-scoring blocks on the 0.70-weighted term and cannot realistically reach the top `limit`. `vec_scores` still carries every cosine into the merge.

- **PostgreSQL (sync adapter): the tsvector trigger check is scoped to the schema's table.** `PostgreSQLAdapter.initialize()` still checked `pg_trigger` by name only (the async adapter was fixed in an earlier release), so a second schema in the same database got no trigger and its `content_tsv` stayed NULL — full-text search silently returned nothing. The check now adds `AND tgrelid = '{schema}.memblock_blocks'::regclass`. Schemas that already hit this have NULL `content_tsv` rows; `UPDATE <schema>.memblock_blocks SET content_tsv = to_tsvector('english', content) WHERE content_tsv IS NULL` repairs them (not run automatically).

### Changed
- **Text-search queries consider a bounded pool of `max(limit * 5, 50)` matches, not every match.** With `sort_by="relevance"` (the default) or `"strength"` the pool is the top keyword/vector-ranked matches and the engine applies its own scoring on top, so results differ from v0.13.1 when more than 50 rows match: previously the newest matching row tended to win, now the best match does. An explicit `sort_by="recency"` / `"access_count"` asks storage for the newest / most-accessed matches instead (`created_at` / `access_count` order), so those results are unchanged from v0.13.1. Queries without `text_search` still fetch the full candidate set (the Python-side strength sort needs it).
- **Hybrid queries on stores without pgvector hydrate at most the pool, not every embedded block** (see Fixed).
- **Equal-score matches come back newest-first on every backend.** SQLite orders by `fts.rank, b.created_at DESC` (the tiebreak costs a temp b-tree over the matched rows — roughly 3 ms → 8 ms at 10k rows / 2,800 matches — but without it equal-bm25 rows came back in FTS5 insertion order, oldest first, so an updated fact lost to the stale one it superseded); PostgreSQL already ordered `ts_rank(...) DESC, created_at DESC`.
- **`sort_by="strength"` with `text_search` still considers every match.** Strength has no SQL equivalent, so the engines do not push the candidate limit for it (v0.13.1 semantics); every other sort works on the bounded pool.
- **`min_strength` refills from the full match set.** The pool is cut in SQL before the Python-side `min_strength` filter; if decayed best matches empty it so that `limit` cannot be filled although the pool was full, the engines re-query without the limit (v0.13.1 behaviour) instead of under-filling.
- **Vector-only hybrid hits and graph neighbours now respect the query's filters.** Blocks fetched outside the candidate SQL (pgvector / brute-force vector hits not in the FTS pool, `related_to` neighbours) are checked against `type`, `session_id` / `org_id` / `project_id` / `agent_id`, `min_confidence`, `tags`, `metadata_filters` and the temporal filters before they can enter the results. Previously (v0.13.1 too) a session-scoped hybrid query could return a block from another session because its vector was close.
- **An inferred temporal window never empties the result set.** The temporal parser turns phrases such as "last week" or "10 days ago" into a `happened_at` window relative to *now*. When nothing falls inside it — every memory predates the window, or the blocks carry no `happened_at` — both engines now retry the search without the inferred window instead of returning nothing. v0.13.1 only avoided the empty result through the unfiltered vector-hydration leak fixed above (FTS-only stores did return nothing). Explicit `happened_after` / `happened_before` filters are never relaxed. This is what kept LongMemEval's temporal-reasoning questions answerable when the fix closed the leak.
- **A `text_search` with no word characters matches nothing on PostgreSQL as well as SQLite.** Previously the PostgreSQL adapters dropped the condition and returned the newest rows.
- **`brute_force_similarity` ranks NaN (corrupt) embeddings last on both the numpy and the pure-Python path.**
- New optional extra `pip install "memblock[fast]"` installs numpy for the SQLite brute-force vector fast path.

### Measured
`benchmarks/latency_bench.py` (new): synthetic 10-fund × 5-topic corpus, hot query = fund + topic (matches ~28 % of rows, ~2 % contain both terms), `limit=10`, on-disk SQLite, Python 3.14 (x86_64 build), numpy 2.5, macOS, idle machine. BEFORE = v0.13.1 (`a11fd7c`) from a clean `git worktree`; AFTER = this release (including the `created_at` tiebreak) — same script, seed (1234), corpus, arguments and interpreter, run back-to-back.
- SQLite FTS-only, 10,000 blocks, hot query P50 / P95: BEFORE 61.2 ms / 64.6 ms → AFTER 10.2 ms / 10.7 ms (6x faster; without the newest-first tiebreak this was 4.7 ms)
- SQLite FTS-only, 1,000 blocks, hot query P50 / P95: BEFORE 6.24 ms / 6.46 ms → AFTER 1.99 ms / 2.22 ms (3.1x)
- Hybrid (FTS + brute-force cosine, 384-d, no pgvector), 10,000 blocks, hot query P50 / P95: BEFORE 762 ms / 771 ms → AFTER 43.5 ms / 47.1 ms (18x)
- Hybrid, 1,000 blocks, hot query P50 / P95: BEFORE 75.4 ms / 76.0 ms → AFTER 5.60 ms / 6.78 ms (13x)
- Ranking, FTS-only: top-1 contains both query terms for 6 % of hot queries and 0.66–0.72 of the top-10 rows do on average → AFTER 100 % and 10.0 of 10 (hybrid was already 100 % / 10.0 at 10k; at 1k its top-10 improves from 6.4 to 10.0 both-terms rows)
- Selective query (unique token, exactly one matching row) P50: BEFORE 0.06 ms → AFTER 0.15 ms (the rank sort, tiebreak and derived-table join add roughly a tenth of a millisecond to a one-row match)
- Store throughput @ 10,000: 740 / 671 blocks/s BEFORE vs 685 / 614 AFTER (FTS-only / with embeddings; the write path is untouched, differences are run-to-run noise)

### Tests
- New: `tests/test_fts_ranking_sqlite.py` — adapter-level: rank ordering (incl. newest-first ties), LIMIT after every WHERE condition (no under-fill), and `EXPLAIN QUERY PLAN` assertions on a deliberately un-analysed database (with planner statistics even the old shape plans FTS-first, so the guard would never trip) that the FTS match is the outer loop and never driven from `idx_blocks_deleted`; the timing guard uses plain wall-clock asserts rather than a process-killing `faulthandler` timer.
- Review-pass regressions (`tests/test_fts_ranking_e2e.py::TestReviewRegressions`, `tests/test_fts_ranking_postgres.py::TestReviewRegressions`, `tests/test_vector_fastpath.py`, `tests/test_embeddings.py`): `min_strength` refill, `strength` sort over every match, session-scoped hybrid queries never leak vector-only hits, the reranker receives `limit * 3` rows from the relevance pool, the per-schema `trg_memblock_tsv` trigger (a second schema used to get no trigger and NULL `content_tsv`), no-word searches on PostgreSQL, NaN embeddings, and the empty-vector RRF curve.
- New: `tests/test_query_pool.py` — engine-level: text searches push `sort_by="relevance"` and the bounded pool down to storage, non-text queries push neither, FTS-only order drives `relevance` scoring, hybrid ranking unchanged, sync and async identical; extended in this release with the explicit-sort mapping, the FTS-only curve beating age, and the bounded hybrid hydration (sync and async).
- New: `tests/test_vector_fastpath.py` — `brute_force_similarity` numpy vs pure-Python agreement (1e-5, identical order) and every fallback condition.
- New: `tests/test_fts_ranking_postgres.py` — adapter-level on live Postgres (psycopg and asyncpg; skipped without `MEMBLOCK_TEST_DB_URL`): `ts_rank` ordering with the `created_at` tiebreak, LIMIT after every WHERE condition, soft-deleted rows never surface, and `EXPLAIN` assertions that the GIN index on `content_tsv` serves the predicate at 10k rows.
- New: `tests/test_fts_ranking_e2e.py` — through the public API on `sqlite:///:memory:` (plus the sync/async engines on a live Postgres schema): a 30-day-old exact-phrase block outranks 40 newer one-token blocks via `MemBlock.query` and `AsyncMemBlock.query`; hybrid with deterministic vectors puts the keyword-best and the semantically-closest rows (different blocks) in the top 3; `build_context(query=...)` lists the best match first; a type-filtered text search with a limit still fills; `sort_by="recency"` is still newest-first.
- Fixed: `tests/test_connection_pool.py` / `tests/test_pgvector.py` reloaded `memblock.storage.postgresql` under a `MagicMock` psycopg and left it mocked for the rest of the session, so later live-Postgres tests saw a mock connection; an autouse fixture now reloads the real module after each test.
- New: `benchmarks/latency_bench.py` — standalone CLI (`--n`, `--queries`, `--mode fts|hybrid|both`, `--db`, `--seed`, `--json`) that imports memblock from its own checkout, so the same script yields BEFORE numbers from a `git worktree` of the previous release and AFTER numbers from the working tree.

### Migration notes
- **Backward compatible API.** No public signatures change; `sort_by="relevance"` is a new accepted value on the storage adapters' `query_blocks` filters and is what the engines now send for text searches. Result *order* for text searches changes as described under Changed; callers that depended on newest-first text-search results should pass `sort_by="recency"` explicitly.
